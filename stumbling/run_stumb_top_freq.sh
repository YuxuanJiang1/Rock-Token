#!/usr/bin/env bash
# W4 ablation (Reviewer RNAB): freeze the top-K tokens by raw Freq(v) alone,
# ignoring mean KL entirely -- tests whether frequency alone explains the
# Fig. 5 gap between "Ours" (Rock-Freeze) and "Original OPD".
# Token list built by build_ablation_freeze_lists.py -> top_freq.json.
#
...
SAVE_DIR=/workspace/Rock-Token-checkpoints/top_freq
...
$PYTHON -m kdflow.cli.train_kd_on_policy \
  ...
  --token_freeze_path "${SCRIPT_DIR}/ablation_lists/top_freq.json" \
  --freeze_weight 0.0 \
  --kd_loss_fn rkl \
  --teacher_tp_size 2 \
