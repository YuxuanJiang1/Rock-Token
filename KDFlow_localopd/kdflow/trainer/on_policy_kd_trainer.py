    ...
    def fit(self, global_step=0, start_epoch=0):
        ...
        for epoch in range(start_epoch, self.epochs):
            ...
            for prompt_batch in self.train_dataloader:
                ...
                print("[ORIG] rollout_samples len =", len(rollout_samples), flush=True)
                print("[ORIG] rollout sample keys =", list(rollout_samples[0].keys()), flush=True)
                ...
                print("[ORIG] teacher forward len =", len(rollout_samples_for_kd), flush=True)
                print("[ORIG] teacher mb keys =", list(rollout_samples_for_kd[0].keys()), flush=True)
                ...
                for i in range(0, len(rollout_samples), num_micro_batches):
                    ...
                    print("[ORIG] global_batch len =", len(global_batch), flush=True)
                    print("[ORIG] stu_loss_mask sum =", global_batch[0]["stu_loss_mask"].sum(), flush=True) 
                ...
                if self.args.train.enable_sleep:
                    print("[ORIG] before student.wakeup", flush=True)
                    self.student.wakeup()
                    print("[ORIG] after student.wakeup", flush=True)
