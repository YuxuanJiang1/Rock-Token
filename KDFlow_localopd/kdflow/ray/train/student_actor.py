        ...
        for batch in train_data:
            ...
            for k, v in micro_batch.items():
                if isinstance(v, torch.Tensor):
                    print(f"[MB] {k}: device={v.device}, shape={tuple(v.shape)}", flush=True)
            ...
            for k, v in loss_info.items():
                if isinstance(v, torch.Tensor):
                    print(f"[LOSS_INFO] {k}: device={v.device}, shape={tuple(v.shape)}", flush=True)

            loss = loss_info["loss"]
            print("[LOSS] loss.device =", loss.device, flush=True)
            
            loss = loss_info["loss"]

            for name, p in self.student.named_parameters():
                if p.requires_grad and p.device.type != "cuda":
                    print("[CPU PARAM][student]", name, p.device, flush=True)

            if hasattr(self.kd_algorithm, "named_parameters"):
                for name, p in self.kd_algorithm.named_parameters():
                    if p.requires_grad and p.device.type != "cuda":
                        print("[CPU PARAM][kd_algorithm]", name, p.device, flush=True)

            for i, group in enumerate(self.optim.param_groups):
                for j, p in enumerate(group["params"]):
                    if p.requires_grad and p.device.type != "cuda":
                        print(f"[CPU PARAM][optimizer group {i} param {j}]", p.device, p.shape, flush=True)
